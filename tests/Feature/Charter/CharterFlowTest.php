...
class CharterFlowTest extends TestCase
{
    ...
    public function test_acente_create_view_does_not_render_preset_package_selection_ui(): void
    {
        $acente = User::factory()->create([
            'role' => 'acente',
            'email' => 'agency-preset-view@example.com',
            'phone' => '905551110010',
        ]);
        ...
        $response->assertOk();
        $response->assertDontSee('Hazir Paket Secimi');
        $response->assertDontSee('js-preset-package-card');
        $response->assertDontSee('data-package=');
    }

    public function test_acente_create_view_does_not_render_transfer_widget_markup(): void
    {
        $acente = User::factory()->create([
            'role' => 'acente',
            'email' => 'agency-transfer-widget@example.com',
            'phone' => '905551110012',
        ]);
        ...
        $response->assertDontSee('id="transferWidgetOpenLink"', false);
        $response->assertDontSee('id="transferWidgetInlineBtn"', false);
    }

    public function test_selected_preset_package_is_persisted_with_snapshot(): void
    {
        $acente = User::factory()->create([
            'role' => 'acente',
            'email' => 'agency-preset-save@example.com',
            'phone' => '905551110011',
        ]);

        CharterPresetPackage::query()->create([
            'code' => 'preset-save-test',
            'title' => 'IST - AYT Save Test',
            'summary' => 'Snapshot kayit testi',
            'transport_type' => CharterRequest::TYPE_JET,
            'from_iata' => 'IST',
            'to_iata' => 'AYT',
            'from_label' => 'Istanbul Airport',
            'to_label' => 'Antalya Airport',
            'aircraft_label' => 'Cessna Citation CJ2',
            'suggested_pax' => 7,
            'trip_type' => 'Tek Yon',
            'group_type' => 'Kurumsal',
            ...
            'price' => 18250,
            'currency' => 'EUR',
            'highlights_json' => ['Hizli onay', 'Bagaj destegi'],
            'is_active' => true,
            'sort_order' => 20,
        ]);
        ...
        $this->actingAs($acente)->post(route('acente.charter.store'), [
            'transport_type' => CharterRequest::TYPE_JET,
            'preset_package_code' => 'preset-save-test',
            'from_iata' => 'SAW',
            'to_iata' => 'BJV',
            'departure_date' => now()->addDays(12)->toDateString(),
            'pax' => 5,
            'is_flexible' => 0,
            'notes' => 'Preset kayit testi',
        ])->assertRedirect();
        ...
        $this->assertSame('preset-save-test', (string) $request->preset_package_code);
        $this->assertSame('IST - AYT Save Test', (string) $request->preset_package_title);
        $this->assertSame('EUR', (string) $request->preset_package_currency);
        $this->assertSame(18250.0, (float) $request->preset_package_price);
        $this->assertSame('Snapshot kayit testi', (string) data_get($request->preset_package_snapshot, 'summary'));
        $this->assertSame('IST', (string) data_get($request->preset_package_snapshot, 'from_iata'));
        $this->assertSame('AYT', (string) data_get($request->preset_package_snapshot, 'to_iata'));
        $this->assertSame('Cessna Citation CJ2', (string) data_get($request->preset_package_snapshot, 'aircraft_label'));
        $this->assertSame('Kurumsal', (string) data_get($request->preset_package_snapshot, 'group_type'));
        $this->assertSame(
            ['Hizli onay', 'Bagaj destegi'],
            array_values((array) data_get($request->preset_package_snapshot, 'highlights', []))
        );
    }
