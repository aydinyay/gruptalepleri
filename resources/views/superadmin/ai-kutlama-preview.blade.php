...
<head>
    ...
    <style>
        ...
        .meta { color:rgba(255,255,255,0.7); font-size:0.85rem; overflow-wrap:anywhere; word-break:break-word; }
        .preview-content h3,
        .preview-content p { overflow-wrap:anywhere; word-break:break-word; }
    </style>
</head>
<body>
<div class="preview-shell">
    <div class="preview-card">
        @if($campaign->image_path)
            <img src="{{ $campaign->image_path }}" alt="AI Kutlama Görseli" class="preview-image">
        @endif
        <div class="preview-content">
            ...
            <h3 class="mb-2">{{ $campaign->title ?: 'Başlık boş' }}</h3>
            <p class="mb-3">{{ $campaign->message ?: 'Mesaj boş' }}</p>
            <div class="d-flex flex-wrap gap-2 align-items-center">
                @if($campaign->cta_text)
                    <a href="{{ $campaign->cta_url ?: '#' }}" class="btn btn-danger btn-sm">{{ $campaign->cta_text }}</a>
                @endif
                <span class="meta">Yayın: {{ $campaign->publish_starts_at?->format('d.m.Y H:i') ?: '-' }} - {{ $campaign->publish_ends_at?->format('d.m.Y H:i') ?: '-' }}</span>
            </div>
        </div>
    </div>
</div>
