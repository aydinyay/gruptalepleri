...
.btn-danger { display:block; background:#dc3545; color:#fff; text-decoration:none; font-weight:700; font-size:15px; padding:14px 36px; border-radius:10px; border:none; cursor:pointer; width:100%; }
...
<div class="card">
    ...
    <p>
        <strong>{{ $abone->email }}</strong> adresine gönderilen e-postaları
        artık almak istemiyorsanız lütfen aşağıdaki butona tıklayın.
    </p>
    <form method="POST" action="{{ route('abone.iptal.onayla', $abone->token) }}">
        @csrf
