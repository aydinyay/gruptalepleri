                    ...
                    @if($charterRequest->preset_package_title)
                        <div class="border rounded-3 p-3 mt-3 bg-light-subtle">
                            <div class="d-flex flex-wrap align-items-center justify-content-between gap-2">
                                <div>
                                    <div class="text-muted small">Hazir Paket</div>
                ...
                <div class="card-body d-grid gap-2">
                    ...
                    @if($charterRequest->booking)
                        ...
                        @if((float) $charterRequest->booking->remaining_amount > 0.0001)
                            <form method="POST" action="{{ route($routePrefix . '.payments.gateway-start', $charterRequest->booking) }}">
                                @csrf
                                <button class="btn btn-primary w-100">Odemeye Gec (Paynkolay)</button>
                            </form>
                            <div class="small text-muted">
                                Tam odeme tutari: {{ number_format((float) $charterRequest->booking->remaining_amount, 2, ',', '.') }}
                            </div>
                        @endif
