...
<head>
    ...
    <style>
        ...
        .gyg-media-item img, .gyg-media-item video { width: 100%; height: 100%; object-fit: cover; transition: transform 0.3s ease; background-color:#e2e8f0; }
        .gyg-media-item:hover img, .gyg-media-item:hover video { transform: scale(1.03); }
        ...
        /* Medya sayısına göre otomatik hizalama */
        ...
        .gyg-gallery[data-count="4"] { grid-template-columns: 2fr 1fr 1fr; grid-template-rows: 1fr 1fr; }
        .gyg-gallery[data-count="4"] .m-0 { grid-row: span 2; }
        .gyg-gallery[data-count="4"] .m-1 { grid-column: span 2; }
        ...
        .form-control:focus { background: #fff; border-color: #0071eb; box-shadow: 0 0 0 4px rgba(0, 113, 235, 0.1); outline: none; }

        /* SAĞ KOLON - Sticky Kutu */
        .sticky-sidebar { position: sticky; top: 20px; }
        .price-card { border: 1px solid #e2e8f0; border-radius: 16px; padding: 1.8rem; background: #fff; box-shadow: 0 15px 35px rgba(0,0,0,0.06); }
        ...
        .btn-submit:hover { background-color: #005bbd; transform: translateY(-2px); box-shadow: 0 8px 20px rgba(0,113,235,0.2); }
    ...
    <form method="POST" action="{{ $bookEndpoint ?? '#' }}" id="checkoutForm">
        ...
        <div class="row g-5">
            ...
            <div class="col-12 col-lg-8">
                ...
                <h1 class="page-title">Özel Transfer: {{ $quote->airport?->code ?? 'Havalimanı' }} ➔ {{ $quote->zone?->name ?? 'Bölge' }}</h1>
                <div class="page-meta">
                    <i class="fas fa-star"></i> 5.0 (Memnuniyet garantisi) • Tedarikçi: <span class="fw-bold text-dark">{{ $quote->supplier?->company_name ?? 'Premium Tedarikçi' }}</span>
                </div>
                ...
                @php
                    ...
                    if ($quote->vehicleType) {
                        ...
                        // Video varsa önce ekle
                        $videoMedia = $vt->media->where('media_type', 'video')->where('is_active', true)->first();
                        if ($videoMedia && $videoMedia->resolvedUrl()) {
                            $mediaUrls[] = $videoMedia->resolvedUrl();
                        }
                    ...
                    if ($totalMedia === 0) {
                        $vehicleNameUrl = urlencode($quote->vehicleType?->name ?? 'Transfer Aracı');
                        $mediaUrls[] = "https://placehold.co/1200x600/f8fafc/475569?text={$vehicleNameUrl}";
                        $totalMedia  = 1;
                    }
                ...
                <div class="gyg-gallery" data-count="{{ $displayCount }}">
                    @foreach(array_slice($mediaUrls, 0, 5) as $index => $mediaUrl)
                        <div class="gyg-media-item m-{{ $index }}">

                            @if(\Illuminate\Support\Str::endsWith(strtolower($mediaUrl), ['.mp4', '.webm', '.mov']))
                                <video src="{{ $mediaUrl }}" autoplay muted loop playsinline></video>
                            @else
                                <img src="{{ $mediaUrl }}" alt="Transfer Aracı {{ $index + 1 }}" onerror="this.src='https://placehold.co/400x300/e2e8f0/475569?text=Gorsel+Yok';">
                            @endif

                            @if($index == 4 && $totalMedia > 5)
                ...
                <div class="gyg-info-list">
                    <div class="gyg-info-item">
                        ...
                        <div class="gyg-info-text">
                            ...
                            <span>Transferden 24 saat öncesine kadar iptal et ve paranının tamamını geri al.</span>
                        </div>
                    </div>
                    <div class="gyg-info-item">
                    ...
                    <div class="gyg-info-item">
                        ...
                        <div class="gyg-info-text">
                            <strong>{{ $quote->vehicleType?->name ?? 'Özel Araç' }} (Sizin Talebiniz: {{ $quote->pax ?? 1 }} Yolcu)</strong>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="passenger_names" class="form-control" rows="2" placeholder="Örn: Ali Yılmaz, Ayşe Kaya vb.">{{ old('passenger_names') }}</textarea>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label">Uçuş Numarası</label>
                        <input type="text" name="flight_number" class="form-control" value="{{ old('flight_number') }}" placeholder="Örn: TK1983">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Terminal Bilgisi</label>
                        <input type="text" name="terminal" class="form-control" value="{{ old('terminal') }}" placeholder="Örn: Dış Hatlar">
                    </div>

                    <div class="col-12">
                        <label class="form-label">Karşılama Tabelasında Yazacak İsim</label>
                        <input type="text" name="pickup_sign_name" class="form-control" value="{{ old('pickup_sign_name', auth()->user()?->name ?? '') }}" placeholder="Örn: GrupTalepleri VIP">
                    </div>

                    <div class="col-12">
                        <label class="form-label">Tam Alış / Bırakış Adresi (Otel vb.)</label>
                        <textarea name="exact_pickup_address" class="form-control" rows="2" placeholder="Örn: Beşiktaş / İstanbul" required>{{ old('exact_pickup_address') }}</textarea>
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="notes" class="form-control" rows="2" placeholder="Örn: Bebek arabamız var vb.">{{ old('notes') }}</textarea>
                    </div>
                </div>
            ...
            <div class="col-12 col-lg-4">
                <div class="sticky-sidebar">
                    <div class="price-card">
                        ...
                        <div class="price-amount">{{ number_format((float) ($quote->total_amount ?? 0), 2, ',', '.') }} {{ $quote->currency ?? 'TRY' }}</div>
                        ...
                        <div class="text-center mt-3">
                            <a href="{{ $searchRoute ?? '#' }}" class="text-primary text-decoration-none fw-bold" style="font-size: 0.9rem;">
                                <i class="fas fa-arrow-left me-1"></i> Aramaya Geri Dön
                            </a>
                        </div>

                        <ul class="trust-list border-top pt-4 mt-4">
                            <li><i class="fas fa-check-circle"></i> 24 saat öncesine kadar iptal et ve paranının tamamını geri al</li>
                            <li><i class="fas fa-check-circle"></i> Seyahat planlarını esnek tut — yerini ayırt ve bugün hiçbir şey ödeme (Cari limit)</li>
                            <li><i class="fas fa-check-circle"></i> Sürpriz ücret yok, her şey dahil hizmet</li>
                        </ul>
...
(() => {
    ...
    const tick = () => {
        ...
        badge.textContent = minutes > 0 ? `Yerinizi tutuyoruz: ${minutes} dk ${seconds} sn` : `Yerinizi tutuyoruz: ${seconds} sn`;

        if (remaining <= 0) {
            clearInterval(timerInterval);
            badgeContainer.style.backgroundColor = '#fef2f2';
