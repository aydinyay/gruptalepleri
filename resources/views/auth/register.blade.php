            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <div style="display:flex;gap:6px;">
                        <input type="text" name="tursab_no" id="tursab_no" class="form-control"
                               value="{{ old('tursab_no') }}" placeholder="12345" style="flex:1;">
                        <button type="button" id="tursab-sorgula-btn"
                                onclick="tursabSorgula()"
                                style="background:#e94560;color:#fff;border:none;border-radius:8px;padding:0 12px;font-size:0.78rem;font-weight:600;white-space:nowrap;cursor:pointer;">
                            Sorgula
                        </button>
                    </div>
                    <div id="tursab-msg" style="font-size:0.75rem;margin-top:4px;display:none;"></div>
                </div>
                <div class="col-md-6">
...
async function tursabSorgula() {
    const belgeNo = document.getElementById('tursab_no').value.trim();
    const msg     = document.getElementById('tursab-msg');
    const btn     = document.getElementById('tursab-sorgula-btn');

    if (!belgeNo) { msg.style.display='block'; msg.style.color='#ff8a9a'; msg.textContent='Belge no giriniz.'; return; }

    btn.disabled = true; btn.textContent = '...';
    msg.style.display = 'none';

    try {
        const res  = await fetch('{{ route("tursab.sorgula") }}?belge_no=' + encodeURIComponent(belgeNo));
        const data = await res.json();

        if (!data.found) {
            msg.style.display='block'; msg.style.color='#ff8a9a';
            msg.textContent = data.message || 'Bulunamadı.';
        } else {
            // Alanları doldur
            const set = (name, val) => { const el = document.querySelector('[name="'+name+'"]'); if(el && val) el.value = val; };
            set('company_title',  data.acente_unvani);
            set('tourism_title',  data.acente_unvani);

            msg.style.display='block'; msg.style.color='#4ade80';
            msg.textContent = '✓ ' + data.acente_unvani + ' bulundu — bilgiler dolduruldu.';
        }
    } catch(e) {
        msg.style.display='block'; msg.style.color='#ff8a9a'; msg.textContent='Bağlantı hatası.';
    } finally {
