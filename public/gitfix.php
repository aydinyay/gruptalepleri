...
// Ürün sayfası hata debug
if (($_GET['action'] ?? '') === 'debug-product') {
    define('LARAVEL_START', microtime(true));
    require $webRoot . '/vendor/autoload.php';
    $app = require_once $webRoot . '/bootstrap/app.php';
    // APP_DEBUG aç, log'u stderr'e yönlendir
    $app->make('config')->set('app.debug', true);
    $kernel = $app->make(\Illuminate\Contracts\Http\Kernel::class);
    $slug = $_GET['slug'] ?? 'kucuk-ozel-yat-1-10-kisi';
    $request = \Illuminate\Http\Request::create('/urun/' . $slug, 'GET', [], [], [], [
        'HTTP_HOST' => 'gruprezervasyonlari.com',
        'HTTPS'     => 'on',
    ]);
    ob_start();
    try {
        ...
        if ($status >= 400) {
            // Exception/error bloğunu bul
            preg_match('/<div class="exception-message[^>]*>(.*?)<\/div>/s', $content, $m);
            if ($m) echo "ERROR: " . strip_tags($m[1]) . "\n";
            // Stack trace
            preg_match_all('/at <em>(.*?)<\/em>(.*?)<\/span>/s', $content, $traces, PREG_SET_ORDER);
            foreach (array_slice($traces, 0, 5) as $t) {
                echo "  " . strip_tags($t[1]) . strip_tags($t[2]) . "\n";
            }
            // Tüm içeriği de göster (ilk 3000 karakter)
            echo "\n--- RESPONSE (3000 chars) ---\n" . substr(strip_tags($content), 0, 3000);
        } else {
            echo "OK - page loaded successfully (" . strlen($content) . " bytes)\n";
        }
    } catch (\Throwable $e) {
        ob_end_clean();
        header('Content-Type: text/plain');
